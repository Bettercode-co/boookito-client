...
export default function Verify() {
  ...
  const username = router.query.usernameData

  const handleSubmit = async (e) => {
    e.preventDefault()
    await AxiosInstance.post('/sendotp',
      JSON.stringify({ username, phonenumber }),
      {
        headers: {
      ...
      if (response.data.success) {
        ...
        setShowToast(true)
        setTimeout(() => {
          router.push({ pathname: '/verify', query: { usernameData: username } })
        }, 2000)
      ...
      <div className="border border-gray-200 p-10 rounded-lg max-w-md w-full space-y-8">
        <div>
          ...
          <p className="text-center text-sm text-gray-600 mt-5">
            <a href="#" className="font-medium text-blue-600 hover:text-indigo-500"> شماره موبایل معتبر خود را وارد کنید </a>
          </p>
        </div>
        <div className="mt-8 space-y-6">
          {showToast && <Toast type='success' message='کد ارسال شد' />}
          <form onSubmit={handleSubmit}>
            <div className="rounded-md shadow-sm -space-y-px">
              <div>
                <input type="hidden" name='username' value={username} />
                <label htmlFor="email-address" className="sr-only">شماره موبایل</label>
                <input  maxLength={11} value={phonenumber} onChange={(e) => setPhoneNumber(e.target.value)} type="text" autoComplete="off" required className="text-center h-12 appearance-none rounded-lg  relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900  focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="شماره موبایل" />
              </div>
            </div>
            ...
            <div>
              <button type="submit" className={phonenumber.length == 11 ? "group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-blue-500 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
